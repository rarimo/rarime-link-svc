...
var colsProof = `id, creator, created_at, proof, org_id, type, schema_url, operator, field`
...
func (q ProofQ) InsertCtx(ctx context.Context, p *data.Proof) error {
	...
	sqlstr := `INSERT INTO public.proofs (` +
		`id, creator, created_at, proof, org_id, type, schema_url, operator, field` +
		`) VALUES (` +
		`$1, $2, $3, $4, $5, $6, $7, $8, $9` +
		`)`
	// run
	err := q.db.ExecRawContext(ctx, sqlstr, p.ID, p.Creator, p.CreatedAt, p.Proof, p.OrgID, p.Type, p.SchemaURL, p.Operator, p.Field)
	return errors.Wrap(err, "failed to execute insert query")
}
...
func (q ProofQ) UpdateCtx(ctx context.Context, p *data.Proof) error {
	...
	sqlstr := `UPDATE public.proofs SET ` +
		`creator = $1, proof = $2, org_id = $3, type = $4, schema_url = $5, operator = $6, field = $7 ` +
		`WHERE id = $8`
	// run
	err := q.db.ExecRawContext(ctx, sqlstr, p.Creator, p.Proof, p.OrgID, p.Type, p.SchemaURL, p.Operator, p.Field, p.ID)
	return errors.Wrap(err, "failed to execute update")
}
...
func (q ProofQ) UpsertCtx(ctx context.Context, p *data.Proof) error {
	...
	sqlstr := `INSERT INTO public.proofs (` +
		...
		` ON CONFLICT (id) DO ` +
		`UPDATE SET ` +
		`creator = EXCLUDED.creator, proof = EXCLUDED.proof, org_id = EXCLUDED.org_id, type = EXCLUDED.type, schema_url = EXCLUDED.schema_url, operator = EXCLUDED.operator, field = EXCLUDED.field `
	// run
	if err := q.db.ExecRawContext(ctx, sqlstr, p.ID, p.Creator, p.CreatedAt, p.Proof, p.OrgID, p.Type, p.SchemaURL, p.Operator, p.Field); err != nil {
		return errors.Wrap(err, "failed to execute upsert stmt")
	}
	return nil
}
